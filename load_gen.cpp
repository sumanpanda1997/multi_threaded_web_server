/* run using: ./load_gen localhost <server port> <number of concurrent users>
   <think time (in s)> <test duration (in s)> */

#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <netdb.h>

#include <pthread.h>
#include <sys/time.h>

#include <string>
#include <cstring>

int time_up;
FILE *log_file;

// user info struct
struct user_info {
  // user id
  int id;

  // socket info
  int portno;
  char *hostname;
  float think_time;

  // user metrics
  int total_count;
  float total_rtt;

  user_info(int id, int portno, char *hostname, float think_time)
  {
    this->id = id;
    this->portno = portno;
    this->hostname = hostname;
    this->think_time = think_time;
    this->total_count = 0;
    this->total_rtt = 0;
  }
};

// error handling function
void error(char *msg) {
  perror(msg);
  exit(0);
}

// time diff in seconds
float time_diff(struct timeval *t2, struct timeval *t1) {
  return (t2->tv_sec - t1->tv_sec) + (t2->tv_usec - t1->tv_usec) / 1e6;
}

// user thread function
void *user_function(void *arg) {
  /* get user info */
  struct user_info *info = (struct user_info *)arg;

  int sockfd, n;
  char buffer[1024];
  struct timeval start, end;

  struct sockaddr_in serv_addr;
  struct hostent *server;

  while (1) {

    /* if time up, break */
    if (time_up)
      break;

    /* start timer */
    gettimeofday(&start, NULL);    

    /* TODO: create socket */
    sockfd = socket(AF_INET, SOCK_STREAM, 0);
    if (sockfd < 0)
      error("ERROR opening socket");


    /* TODO: set server attrs */
    server = gethostbyname(info->hostname);
    if (server == NULL) 
    {
      fprintf(stderr, "ERROR, no such host\n");
      exit(0);
    }

    bzero((char *)&serv_addr, sizeof(serv_addr));
    serv_addr.sin_family = AF_INET;

    bcopy((char *)server->h_addr, (char *)&serv_addr.sin_addr.s_addr, server->h_length);
    serv_addr.sin_port = htons(info->portno);



    /* try connecting to server until it is accepted from server side*/
    while(connect(sockfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0)
      perror("ERROR connecting, now retrying");

    /* TODO: send message to server */

    std::string request = "GET / HTTP/1.1\nHost: localhost:8080\nUser-Agent: curl/7.81.0";
    
    // try sending until it is successful 
    n = write(sockfd, request.c_str(), request.size());
    if (n <= 0)
    {
	    perror("ERROR writing to socket, close the socket and going for another try"); 
      close(sockfd);
      continue;
    }


    /* TODO: read reply from server */

    bzero(buffer, sizeof(buffer));
    n = read(sockfd, buffer, sizeof(buffer));	
		if (n < 0)
    {
		  perror("ERROR reading from socket, closing the socket and going for another try");	
      close(sockfd);
      continue;
    }
	  // printf("From Server : %s\n", buffer);

    /* TODO: close socket */
  	close(sockfd);
    /* end timer */
    gettimeofday(&end, NULL);


    /* TODO: update user metrics */
    info->total_count += 1;
    info->total_rtt += time_diff(&end, &start);
    /* TODO: sleep for think time */
    usleep(info->think_time*1000000);
  }

  /* exit thread */
  // fprintf(log_file, "User #%d finished\n", info->id);
  fflush(log_file);
  pthread_exit(NULL);
}

int main(int argc, char *argv[]) {
  int user_count, portno, test_duration;
  float think_time;
  char *hostname;

  

  if (argc != 6) {
    fprintf(stderr,
            "Usage: %s <hostname> <server port> <number of concurrent users> "
            "<think time (in s)> <test duration (in s)>\n",
            argv[0]);
    exit(0);
  }

  hostname = argv[1];
  portno = atoi(argv[2]);
  user_count = atoi(argv[3]);
  think_time = atof(argv[4]);
  test_duration = atoi(argv[5]);

  // fprintf(stdout, "Hostname: %s\n", hostname);
  // fprintf(stdout, "Port: %d\n", portno);
  // fprintf(stdout, "User Count: %d\n", user_count);
  // fprintf(stdout, "Think Time: %f s\n", think_time);
  // fprintf(stdout, "Test Duration: %d s\n", test_duration);

  /* open log file */
  log_file = fopen("load_gen.csv", "a");

  pthread_t threads[user_count];
  struct user_info *info[user_count];
  struct timeval start, end;

  /* start timer */
  gettimeofday(&start, NULL);
  time_up = 0;
  for (int i = 0; i < user_count; ++i) 
  {
    /* TODO: initialize user info */
    info[i] = new user_info(i, portno, hostname, think_time);
    // fprintf(log_file, "created info %d and think time %f\n", info[i]->id, info[i]->think_time);
    /* TODO: create user thread */
    if( pthread_create(&threads[i], NULL, user_function, info[i]) != 0 ) 
      error("pthread_create() error");
    // fprintf(log_file, "Created thread %d\n", i);
  }

  /* TODO: wait for test duration */
  usleep(test_duration*1000000);

  // fprintf(stdout, "Woke up\n");
  /* end timer */
  time_up = 1;
  gettimeofday(&end, NULL);
  float total_time = time_diff(&end, &start);

  /* TODO: wait for all threads to finish */

  void *status;
  for(int t=0; t<user_count; t++) 
  {
    int rc = pthread_join(threads[t], &status);
    if (rc) 
      error("ERROR; return code from pthread_join() is \n");
    // printf("Main: completed join with thread %d having a status of %ld\n",t,(long)status);
  }
  

   fprintf(stdout, "id,request,rtt\n");
  /* TODO: print results */
  float overall_rtt = 0;
  int overall_req = 0;
  for(int i=0; i<user_count; i++)
  {
    fprintf(stdout, "%d,%d,%f\n", info[i]->id, info[i]->total_count, info[i]->total_rtt);
    overall_req = overall_req + info[i]->total_count;
    overall_rtt = overall_rtt + info[i]->total_rtt;
  }

  /* close log file */
  float avg_throughput = overall_req/total_time;
  float avg_response_time = overall_rtt/overall_req;
  fprintf(log_file, "%d,%f,%f\n",user_count, avg_throughput, avg_response_time);
  fclose(log_file);

  return 0;
}